...
CREATE DATABASE /*!32312 IF NOT EXISTS*/`CarSale` /*!40100 DEFAULT CHARACTER SET latin1 */;
...
CREATE TABLE `Клиенты` (
  `номер_клиент` int(11) NOT NULL,
  `имя_клиента` varchar(50) NOT NULL,
  `контакная_фамилия` varchar(50) NOT NULL,
  `контакная_иия` varchar(50) NOT NULL,
  `телефон` varchar(50) NOT NULL,
  `адресс1` varchar(50) NOT NULL,
  `адресс2` varchar(50) DEFAULT NULL,
  `город` varchar(50) NOT NULL,
  `государство` varchar(50) DEFAULT NULL,
  `почтовый_код` varchar(15) DEFAULT NULL,
  `страна` varchar(50) NOT NULL,
  `номер_работник` int(11) DEFAULT NULL,
  `кредитный_лимит` decimal(10,2) DEFAULT NULL,
  PRIMARY KEY (`номер_клиент`),
  KEY `номер_работник` (`номер_работник`),
  CONSTRAINT `customers_ibfk_1` FOREIGN KEY (`номер_работник`) REFERENCES `Работник` (`номер_работник`)
);

/*Data for the table `customers` */

insert  into `Клиенты`(`номер_клиент`,`имя_клиента`,`контакная_фамилия`,`контакная_иия`,`телефон`,`адресс1`,`адресс2`,`город`,`государство`,`почтовый_код`,`страна`,`номер_работник`,`кредитный_лимит`) values 
...
CREATE TABLE `Работник` (
  `номер_работник` int(11) NOT NULL,
  `фамилия_работника` varchar(50) NOT NULL,
  `имя_работника` varchar(50) NOT NULL,
  `расширение` varchar(10) NOT NULL,
  `почта` varchar(100) NOT NULL,
  `код_офиса` varchar(10) NOT NULL,
  `сообщить` int(11) DEFAULT NULL,
  `название_работы` varchar(50) NOT NULL,
  PRIMARY KEY (`номер_работник`),
  KEY `сообщить` (`сообщить`),
  KEY `код_офиса` (`код_офиса`),
  CONSTRAINT `employees_ibfk_1` FOREIGN KEY (`сообщить`) REFERENCES `Работник` (`номер_работник`),
  CONSTRAINT `employees_ibfk_2` FOREIGN KEY (`код_офиса`) REFERENCES `Офис` (`код_офиса`)
);

/*Data for the table `employees` */

insert  into `Работник`(`номер_работник`,`фамилия_работника`,`имя_работника`,`расширение`,`почта`,`код_офиса`,`сообщить`,`название_работы`) values 
...
CREATE TABLE `Офис` (
  ...
  `государство` varchar(50) DEFAULT NULL,
  `страна` varchar(50) NOT NULL,
  `почтовый_код` varchar(15) NOT NULL,
  `территория` varchar(10) NOT NULL,
  PRIMARY KEY (`код_офиса`)
);

/*Data for the table `offices` */

insert  into `Офис`(`код_офиса`,`город`,`телефон`,`адресс1`,`адресс2`,`государство`,`страна`,`почтовый_код`,`территория`) values 
...
DROP TABLE IF EXISTS `ДетальЗаказ`;

CREATE TABLE `ДетальЗаказ` (
  `номер_заказ` int(11) NOT NULL,
  `код_товара` varchar(15) NOT NULL,
  `заказанное_количество` int(11) NOT NULL,
  `цена_каждого` decimal(10,2) NOT NULL,
  `строки_заказа` smallint(6) NOT NULL,
  PRIMARY KEY (`номер_заказ`,`код_товара`),
  KEY `код_товара` (`код_товара`),
  CONSTRAINT `orderdetails_ibfk_1` FOREIGN KEY (`номер_заказ`) REFERENCES `Заказ` (`номер_заказ`),
  CONSTRAINT `orderdetails_ibfk_2` FOREIGN KEY (`код_товара`) REFERENCES `Товар` (`код_товара`)
);

/*Data for the table `orderdetails` */

insert  into `ДетальЗаказ`(`номер_заказ`,`код_товара`,`заказанное_количество`,`цена_каждого`,`строки_заказа`) values 
...
CREATE TABLE `Заказ` (
  ...
  `требуемая_дата` date NOT NULL,
  `дата_доставки` date DEFAULT NULL,
  `статус` varchar(15) NOT NULL,
  `комментарии` text,
  `номер_клиент` int(11) NOT NULL,
  PRIMARY KEY (`номер_заказ`),
  KEY `номер_клиент` (`номер_клиент`),
  CONSTRAINT `orders_ibfk_1` FOREIGN KEY (`номер_клиент`) REFERENCES `Клиенты` (`номер_клиент`)
);

/*Data for the table `Заказ` */

insert  into `Заказ`(`номер_заказ`,`дата_заказа`,`требуемая_дата`,`дата_доставки`,`статус`,`комментарии`,`номер_клиент`) values 
...
CREATE TABLE `Оплата` (
  `номер_клиент` int(11) NOT NULL,
  `номер_чека` varchar(50) NOT NULL,
  `дата_платежа` date NOT NULL,
  `суммы` decimal(10,2) NOT NULL,
  PRIMARY KEY (`номер_клиент`,`номер_чека`),
  CONSTRAINT `payments_ibfk_1` FOREIGN KEY (`номер_клиент`) REFERENCES `Клиенты` (`номер_клиент`)
);

/*Data for the table `payments` */

insert  into `Оплата`(`номер_клиент`,`номер_чека`,`дата_платежа`,`суммы`) values 
...
/*Table structure for table `тип_продукт` */

DROP TABLE IF EXISTS `Тип_продукты`;

CREATE TABLE `Тип_продукты` (
  `тип_продукт` varchar(50) NOT NULL,
  `текстовое_описание` varchar(4000) DEFAULT NULL,
  `html_описание` mediumtext,
  `изображение` mediumblob,
  PRIMARY KEY (`тип_продукт`)
);

/*Data for the table `productlines` */

insert  into `тип_продукты`(`тип_продукт`,`текстовое_описание`,`html_описание`,`изображение`) values 
...
CREATE TABLE `Товар` (
  `код_товара` varchar(15) NOT NULL,
  `наименование_товара` varchar(70) NOT NULL,
  `тип_продукт` varchar(50) NOT NULL,
  `шкала_товара` varchar(10) NOT NULL,
  `продавец_товар` varchar(50) NOT NULL,
  `описание_товар` text NOT NULL,
  `количество_на_складе` smallint(6) NOT NULL,
  `цена_покупки` decimal(10,2) NOT NULL,
  `MSRP` decimal(10,2) NOT NULL,
  PRIMARY KEY (`код_товара`),
  KEY `тип_продукт` (`тип_продукт`),
  CONSTRAINT `products_ibfk_1` FOREIGN KEY (`тип_продукт`) REFERENCES `Тип_продукты` (`тип_продукт`)
);

/*Data for the table `Товар` */

insert  into `Товар`(`код_товара`,`наименование_товара`,`тип_продукт`,`шкала_товара`,`продавец_товар`,`описание_товар`,`количество_на_складе`,`цена_покупки`,`MSRP`) values 
